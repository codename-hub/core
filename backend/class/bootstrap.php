...
class bootstrap
{
     ...
     * @var string
     */
    public const EXCEPTION_GETMODEL_MODELNOTFOUND = 'EXCEPTION_GETMODEL_MODELNOTFOUND';
    /**
     * The current app's parent has the same name as my current app. I won't do that due to risks of infinite loops.
     * @var string
     */
    public const EXCEPTION_GETMODEL_APPSTACKRECURSIVE = 'EXCEPTION_GETMODEL_APPSTACKRECURSIVE';
    /**
    ...
    public static function getModel(string $model, string $app = '', string $vendor = ''): model
    {
        ...
        // check for existence using autoload capabilities
        ...
        if ($app == 'core') {
            throw new exception(self::EXCEPTION_GETMODEL_MODELNOTFOUND, exception::$ERRORLEVEL_FATAL, ['model' => $model, 'app' => $app, 'vendor' => $vendor]);
        }
        ...
        if ($parentapp == $app) {
            throw new exception(self::EXCEPTION_GETMODEL_APPSTACKRECURSIVE, exception::$ERRORLEVEL_FATAL, ['model' => $model, 'app' => $app, 'vendor' => $vendor]);
        }
     ...
     */
    protected static function array_find(array $xs, callable $f): mixed
    {
        foreach ($xs as $x) {
