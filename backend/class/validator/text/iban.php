...
class iban extends text implements validatorInterface
{
    /**
    ...
    public function validate(mixed $value): array
    {
        ...
        if (strlen($value) != $this->countries[substr($value, 0, 2)]) {
            $this->errorstack->addError('VALUE', 'IBAN_LENGTH_NOT_MATCHING_COUNTRY', $value);
            return $this->errorstack->getErrors();
        }
        ...
        foreach ($MovedCharArray as $key => $value) {
            if (!is_numeric($value) && array_key_exists($value, $this->chars)) {
                $MovedCharArray[$key] = $this->chars[$value];
            }
