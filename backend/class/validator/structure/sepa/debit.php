 ...
 * @since 2016-10-11
 */
class debit extends \codename\core\validator\structure implements \codename\core\validator\validatorInterface {
    ...
    public function validate($value) : array {
        ...
        if(!is_object($value['debitor'])) {
            $this->errorstack->addError('VALUE', 'CONTAINING_DEBITOR_NOT_AN_OBJECT', $value);
            return $this->errorstack->getErrors();
        }

        if(!$value['debitor'] instanceof \codename\core\value\structure\sepa\debitor) {
            $this->errorstack->addError('VALUE', 'CONTAINING_DEBITOR_NOT_AN_INSTANCE_OF_DEBITOR_VALUE_OBJECT', $value);
            return $this->errorstack->getErrors();
        }
