...
class crud extends bootstrapInstance {
    ...
    public function __CONSTRUCT(\codename\core\model $model) {
        $this->model = $model;
        $this->setConfig();
        $this->form = new \codename\core\form(array('form_action' => '?context=' . $this->getRequest()->getData('context') . '&view=' . $this->getRequest()->getData('view'),'form_method' => 'post','form_id' => $this->form_id));
        return $this;
    }
     ...
     * @return \codename\core\crud
     */
    public function setConfig(string $identifier = '') : \codename\core\crud {
     ...
     * @return \codename\core\crud
     */
    public function addModifier(string $field, callable $modifier) : \codename\core\crud {
     ...
     */
    public function setColumnOrder(array $columns = array()) : \codename\core\crud {
     ...
     * @return \codename\core\crud
     */
    public function addRowModifier(callable $modifier) : \codename\core\crud {
     ...
     */
    public function makeForm($primarykey = null, $addSubmitButton = true) : \codename\core\form {
    ...
    public function create() {
        ...
        $form = $this->makeForm();
        $hookval = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_CREATE_FORM_INIT, $form);
        if(is_object($hookval) && $hookval instanceof \codename\core\form) {
            $form = $hookval;
        }
        ...
        // this should be BEFORE_VALIDATION
        $newData = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_CREATE_BEFORE_VALIDATION, $data);
        if(is_array($newData)) {
            $data = $newData;
        }
        ...
        app::getHook()->fire(\codename\core\hook::EVENT_CRUD_CREATE_AFTER_VALIDATION, $data);

        // Fire hook for additional validators
        $errors = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_CREATE_VALIDATION, $data);
        if($errors != null && is_array($errors) && sizeof($errors) > 0) {
          $this->getResponse()->setData('errors', $errors);
          $this->getResponse()->setData('view', 'save_error');
          return;
        }

        app::getHook()->fire(\codename\core\hook::EVENT_CRUD_CREATE_BEFORE_SAVE, $data);
        $this->getMyModel()->save($data);
        app::getHook()->fire(\codename\core\hook::EVENT_CRUD_CREATE_SUCCESS, $data);
    ...
    public function edit($primarykey) {
        ...
        $hookval = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_EDIT_FORM_INIT, $form);
        if(is_object($hookval) && $hookval instanceof \codename\core\form) {
            $form = $hookval;
        }
        ...
        $newData = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_EDIT_BEFORE_VALIDATION, $data);
        if(is_array($newData)) {
            $data = $newData;
        }
        ...
        // Fire hook after successful validation
        app::getHook()->fire(\codename\core\hook::EVENT_CRUD_EDIT_AFTER_VALIDATION, $data);

        // Fire hook for additional validators
        $errors = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_EDIT_VALIDATION, $data);
        if($errors != null && is_array($errors) && sizeof($errors) > 0) {
          $this->getResponse()->setData('errors', $errors);
          $this->getResponse()->setData('view', 'save_error');
          return;
        ...
        $newData = app::getHook()->fire(\codename\core\hook::EVENT_CRUD_EDIT_BEFORE_SAVE, $data);
        if(is_array($newData)) {
            $data = $newData;
        }
        ...
        app::getHook()->fire(\codename\core\hook::EVENT_CRUD_EDIT_SUCCESS, $data);
     ...
     * @return \codename\core\form
     */
    public function getForm() : \codename\core\form {
        return $this->form;
    }
